...
from config import CONTENT_LOCATION, ROOT_LOCATION, TEMPLATE_LOCATION
...
def send_content(context):
    ...
    path = CONTENT_LOCATION.format(context.path)    

    if not os.path.isfile(path):
        path = ROOT_LOCATION.format(context.path)
...
def store_content(context):
    ...
    path = CONTENT_LOCATION.format(context.getvalue("resource"))
